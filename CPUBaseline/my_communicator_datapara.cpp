...
void *model_task(void * id){
    ...
    int mini_batch_size_per_thread = num_mbsamples_per_wk / threads;
    int bias = myRank * num_mbsamples_per_wk + mini_batch_size_per_thread * thread_id;
    ...
    for(int epoch = 0; epoch < numberOfIterations; epoch++){   
        for (int i = 0; i < dr_numSamples; i += mini_batch_size){ 
            ...
            calc_gradient(statisitcs + bias, dr_a_norm_fp + 1l*(i + bias) * num_features_per_wk, num_features_per_wk, mini_batch_size_per_thread, stepSize_in_use, x_gradient[thread_id]);
            pthread_barrier_wait(&barrier);
...
int main(int argc, char** argv){
    ...
    mini_batch_size = mini_batch_size / nRanks * nRanks;
    ...
    num_features_per_wk = dr_numFeatures;//num_features_per_wk = dr_numFeatures / (nRanks);
    num_mbsamples_per_wk = mini_batch_size / nRanks;
	stepSize_in_use = stepSize/(float)mini_batch_size;
    // stepSize_in_use = 1;
    int feature_offset = 0;// int feature_offset = num_features_per_wk * (myRank);
    ...
    float* h_dr_a_norm_fp = (float*)_mm_malloc(1l*dr_numSamples*dr_numFeatures*sizeof(float), 64); 
    ...
    for(int epoch = 0; epoch < numberOfIterations; epoch++){  
        timer.start();
        for (int i = 0; i < dr_numSamples; i += mini_batch_size){ //loop for dr_numSamples/mini_batch_size times
            pthread_barrier_wait(&barrier); 
        ...
        MPICHECK(MPI_Allreduce(x_tmp, total_x_tmp, num_features_per_wk, MPI_FLOAT, MPI_SUM, MPI_COMM_WORLD));
...
free(dot_sum);
// std::cout<<"check point 7"<<"\n";
free(statisitcs);
// std::cout<<"check point 8"<<"\n";
free(h_dr_b);
// std::cout<<"check point 9"<<"\n";
free(h_dr_a_norm_fp);
// std::cout<<"check point 10"<<"\n";
free(x);
free(total_x_tmp);
